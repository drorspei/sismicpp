...
struct Evaluator {
    ...
    virtual std::vector<std::shared_ptr<const Event>> execute_action(const Transition& transition, const Event* event) const = 0;
    virtual std::vector<std::shared_ptr<const Event>> execute_on_entry(const State& state) = 0;
    virtual std::vector<std::shared_ptr<const Event>> execute_on_exit(const State& state) = 0;
    virtual ~Evaluator() {}
};
